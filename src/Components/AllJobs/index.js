...
import {AiOutlinesSearch} from 'react-icons/ai'
...
class AllJobs extends Component {
  ...
  onGetProfileDetails = async () => {
    ...
    if (responseProfile.ok === true) {
      ...
      const updatedDataProfile = fetchedDataProfile.map(eachitem => ({
        ...
        profileImageUrl: eachItem.profile_details.short_bio,
      }))
      this.setState({
  ...
  onGetJobDetails = async () => {
    ...
    const jwtToken = Cookies.get('jwt_token')
    const {checkboxInputs, radioInput, searchInput} = this.state
    const jobApiUrl = `https://apis.ccbp.in/jobs?employment_type=${checkboxInputs}&minimum_package=${searchInput}`
    const optionsJobs = {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
  ...
  onRenderProfileStatus = () => {
    ...
    switch (apiStatus) {
      ...
      case apiStatusConstants.failure:
        return this.onFetProfileFailureView()
  ...
  render() {
    //eslint-disable-next-line no usused-vars 
    const {checkboxInputs, radioInput, searchInput} = this.state
    return (
      <>
        <Header />
        <div className="all-jobs-container">
          <div className="side-bar-container">
            ...
            <h1 className="text">Type of Employments</h1>
